...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (_ip) {
      ...
      const _tries = await _sqlRL`SELECT COUNT(*)::int AS c FROM check_attempts WHERE ip = ${_ip} AND kind = 'aff-forgot' AND created_at > now() - interval '1 hour'`;
      if (_tries[0].c >= 3) return { statusCode: 429, headers, body: JSON.stringify({ ok: false, error: 'محاولات كثيرة — حاول بعد قليل' }) };
      await _sqlRL`INSERT INTO check_attempts (ip, kind) VALUES (${_ip}, 'aff-forgot')`;
    }
    const sql = getSql();
